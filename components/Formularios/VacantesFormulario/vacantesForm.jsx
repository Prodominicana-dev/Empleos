...
const VacantesForm = ({ target, setTarget, setTarget02, setTargetPlus, targetPlus, Id }) => {
  ...
  useEffect(() => {
    async function obtenerVacante() {
      try {
        axios.get(`${Url}Vacantes`).then((response) => {
  ...
  const sourceWithoutTarget = source.filter((sourceItem) => {
    ...
    return !foundInTarget && sourceItem.Condicion !== 'deshabilitado';
  });
  ...
  const sourceItemTemplate = (item) => {
    if (item.Condicion !== 'deshabilitado' && item.Id !== target.Id) {
      return (
        <div
        ...
        >
          <div className="flex-1 flex flex-column gap-2">
            <span className="font-bold">{item.NombreVacante}</span>
            <div className="flex align-items-center gap-2">
              <span>{item.PerfilPuesto}</span>
            </div>
          </div>
      ...
      <div className="flex flex-wrap p-2 align-items-center gap-3 ml-5">
        <div className="flex-1 flex flex-column gap-2">
          <div className="row bm-3">
            <span className="font-bold text-900 col-lg-10">{item.NombreVacante}</span>
