...
//    const [errMovil, setErrMovil] = useState(null);
   const [errPassword, setErrPassword] = useState(null);
    ...
    const handleSubmit = async (e) => {
        ...
        if(name==''||email==''||password==''){
            if(name==''){
                setErrname('Este campo es requerido');
            }
            if(email==''){
        ...
        //console.log(registro)
          ...
          // Realizar la solicitud HTTP POST utilizando Axios
           ...
           await Axios.post(`${Url}Subscription`,registro).then(res=>{
                ...
                <div style={{ borderRadius: '56px', padding: '0.3rem', background: 'linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)' }}>
                    <div className="w-full surface-card py-8 px-5 sm:px-8" style={{ borderRadius: '53px' }}>
                        <div className="text-center mb-5">
                            ...
                            <span className="text-600 font-medium">Registrate</span>
                        ...
                        <label htmlFor="name" className="block text-900 text-xl font-medium mb-2 control-label required">
                            ...
                            <InputText inputid="name" type="text" placeholder="name y Apellido" className="w-full md:w-30rem mb-3 " style={{ padding: '1rem' }} value={name} onChange={(e) => setName(e.target.value)} required />
                            <div  className='mt-1 text-right' style={{color:'#ED265E'}}>{errname}</div>
